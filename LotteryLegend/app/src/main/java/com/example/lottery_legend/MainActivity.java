...
public class MainActivity extends AppCompatActivity {
    ...
    // Use M/d/yyyy format to match CreateEventActivity
    private final SimpleDateFormat storageFormat = new SimpleDateFormat("M/d/yyyy", Locale.getDefault());
        ...
        for (Event event : allEvents) {
            ...
            if (startDateFilter != null) {
                ...
                if (dateStr == null || dateStr.trim().isEmpty()) {
                    ...
                    Date eventDate = parseEventStartDate(dateStr.trim());
                    if (eventDate == null) {
                        matchesDate = false;
                    } else {
                        // Normalize both event date and filter dates to UTC start of day
                        long eventTimeUtc = normalizeToUtcStartOfDay(eventDate.getTime());
    ...
    private Date parseEventStartDate(String dateStr) {
        try {
            // Match CreateEventActivity format M/d/yyyy
            return storageFormat.parse(dateStr);
        } catch (ParseException e) {
            Log.e("MainActivity", "Failed to parse date: " + dateStr, e);
            return null;
        }
    }
